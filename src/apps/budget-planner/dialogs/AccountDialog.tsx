...
import { ACCOUNT_TYPES, accountGroupForType } from '../utils/accounts';
...
export function AccountDialog({ account, onClose }: Props) {
  ...
  const save = () => {
    ...
    const group = accountGroupForType(type);
    const balanceChanged = account ? balance !== account.balance : false;
    const data = {
      name,
      type,
      institution: inst || 'Manual',
      accountNumber: num,
      ...
      visibilityScopes: account?.visibilityScopes ?? { dashboard: true, reports: true, networth: true, budget: true },
      reconciled: balanceChanged ? false : account?.reconciled ?? false,
      lastReconciledAt: balanceChanged ? '' : account?.lastReconciledAt ?? '',
      notes,
  ...
  return (
    ...
    >
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 540, boxShadow: '0 8px 40px rgba(0,0,0,.18)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: C.text2, display: 'block', marginBottom: 5 }}>Last 4 digits</label>
          ...
          <div>
            ...
            <textarea ref={notesRef} rows={3} placeholder="Manual balance source, statement date, or reconciliation note" defaultValue={account?.notes ?? ''}
